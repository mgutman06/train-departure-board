...
try:
    # Attempt to load config data
    from config import ZONE_HOME, LOCATION_HOME, ZONE_OVERHEAD
...
class Overhead:
    ...
    def _grab_data(self):
        ...
        # Grab flight details
        try:
            bounds_home = self._api.get_bounds(ZONE_DEFAULT)
            flights_home = self._api.get_flights(bounds=bounds_home)
            flights_home = [f for f in flights_home if f.altitude < 10000 and f.altitude > MIN_ALTITUDE]

            bounds_big = self._api.get_bounds(ZONE_OVERHEAD)
            flights_big = self._api.get_flights(bounds=bounds_big)
            flights_big = [f for f in flights_big if f.altitude >= 10000]

            flights = flights_home + flights_big
